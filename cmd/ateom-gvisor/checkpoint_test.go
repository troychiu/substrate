...
func TestCheckpointWorkloadReplaysCompletedCheckpoint(t *testing.T) {
	...
	// so a success here can only be the replay. The lock is the one field
	// CheckpointWorkload uses before any of that — it dereferences it on its
	// first line — so it has to be real.
